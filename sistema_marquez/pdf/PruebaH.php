...
class PDF extends FPDF
{
    ...
    function AddInvoiceSection($products, $cliente)
    {
        ...
        // Aquí cambiamos la celda del Número de Orden
        $this->Cell(110, 6, utf8_decode('Dirección: Calle Falsa 123, CABA'), 0, 0, 'L'); // Alineado a la izquierda
        $this->Cell(44, 6, utf8_decode('Numero Orden: 0001'), 0, 1, 'R'); // Alineado a la derecha
        $this->Ln(3);
        ...
        $this->Cell(30, 8, 'IVA (21%)', 1, 0, 'C', true); // Coloca el IVA después del Precio Unitario
        $this->Cell(30, 8, 'Total', 1, 1, 'C', true); // Coloca un '1' para que el fondo se aplique
        ...
        foreach ($products as $producto) {
            // Asegúrate de que cantidad y precio_unitario son números
            ...
            // Si la cantidad o el precio son cero, puedes manejarlo de diferentes maneras
            // Por ejemplo, puedes no mostrar la fila o mostrarla con datos vacíos
            $this->Cell(30, 8, $cantidad, 1);
            $this->Cell(70, 8, $producto['descripcion'], 1);
            $this->Cell(30, 8, '$' . number_format($precio_unitario, 2), 1);
            $this->Cell(30, 8, '$' . number_format($total * 0.21, 2), 1); // Calcular el IVA
            $this->Cell(30, 8, '$' . number_format($total + ($total * 0.21), 2), 1); // Total con IVA

            $this->Ln();
            $totalFactura += $total + ($total * 0.21); // Sumar el total con IVA a la factura
        }

        // Asegúrate de que haya 14 filas
        ...
        $this->Ln(0); // Espacio entre el total y el footer
...
// Asegurarse de que haya 14 filas
